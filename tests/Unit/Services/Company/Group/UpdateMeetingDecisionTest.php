...
use App\Services\Company\Group\UpdateMeetingDecision;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Illuminate\Database\Eloquent\ModelNotFoundException;

class UpdateMeetingDecisionTest extends TestCase
{
    use DatabaseTransactions;

    /** @test */
    public function it_updates_a_meeting_decision_as_administrator(): void
    {
        ...
        $this->executeService($michael, $michael->company, $group, $meeting, $agendaItem, $meetingDecision);
    }

    /** @test */
    public function it_updates_a_meeting_decision_as_hr(): void
    {
    ...
    /** @test */
    public function it_updates_a_meeting_decision_as_normal_user(): void
    {
    ...
    private function executeService(Employee $michael, Company $company, Group $group, Meeting $meeting, AgendaItem $agendaItem, MeetingDecision $meetingDecision): void
    {
        Queue::fake();
        ...
        $meetingDecision = (new UpdateMeetingDecision)->execute($request);
        ...
        Queue::assertPushed(LogAccountAudit::class, function ($job) use ($michael, $group, $meeting) {
            return $job->auditLog['action'] === 'meeting_decision_updated' &&
