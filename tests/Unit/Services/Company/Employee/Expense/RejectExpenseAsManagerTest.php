...
class RejectExpenseAsManagerTest extends TestCase
{
    use DatabaseTransactions;

    /** @test */
    public function it_rejects_an_expense_as_the_manager(): void
    {
    ...
    /** @test */
    public function it_rejects_an_expense_as_the_manager_even_if_the_employee_of_the_expense_doesnt_exist_anymore(): void
    {
        $manager = $this->createAdministrator();
    ...
    public function it_fails_if_wrong_parameters_are_given(): void
    {
        ...
        (new RejectExpenseAsManager)->execute($request);
    }
    ...
    private function executeService(Employee $manager, Employee $employee = null, Expense $expense): void
    {
        ...
        $this->assertDatabaseHas('expenses', [
            'id' => $expense->id,
            'status' => Expense::REJECTED_BY_MANAGER,
            'company_id' => $manager->company_id,
            'manager_rejection_explanation' => 'reason',
        ]);
        ...
        Queue::assertPushed(LogAccountAudit::class, function ($job) use ($manager, $expense) {
            return $job->auditLog['action'] === 'expense_rejected_by_manager' &&
        ...
        Queue::assertPushed(LogEmployeeAudit::class, function ($job) use ($manager, $expense) {
            return $job->auditLog['action'] === 'expense_rejected_for_employee' &&
                $job->auditLog['author_id'] === $manager->id &&
