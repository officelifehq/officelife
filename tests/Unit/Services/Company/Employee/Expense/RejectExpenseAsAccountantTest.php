...
class RejectExpenseAsAccountantTest extends TestCase
{
    use DatabaseTransactions;

    /** @test */
    public function it_rejects_an_expense_as_the_accountant(): void
    {
    ...
    /** @test */
    public function it_rejects_an_expense_as_the_accountant_even_if_the_employee_of_the_expense_doesnt_exist_anymore(): void
    {
        $manager = Employee::factory()->create([
            'can_manage_expenses' => true,
        ]);
    ...
    /** @test */
    public function it_fails_if_the_author_is_not_an_accountant(): void
    {
    ...
    public function it_fails_if_wrong_parameters_are_given(): void
    {
        ...
        (new RejectExpenseAsAccountant)->execute($request);
    }
    ...
    private function executeService(Employee $manager, Employee $employee = null, Expense $expense): void
    {
        ...
        $this->assertDatabaseHas('expenses', [
            ...
            'company_id' => $manager->company_id,
            'accounting_rejection_explanation' => 'reason',
        ]);
        ...
        Queue::assertPushed(LogAccountAudit::class, function ($job) use ($manager, $expense) {
            return $job->auditLog['action'] === 'expense_rejected_by_accounting' &&
        ...
        Queue::assertPushed(LogEmployeeAudit::class, function ($job) use ($manager, $expense) {
            return $job->auditLog['action'] === 'expense_rejected_by_accounting_for_employee' &&
                $job->auditLog['author_id'] === $manager->id &&
