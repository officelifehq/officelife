...
use App\Services\Company\Project\RemoveLabelFromIssue;
use Illuminate\Foundation\Testing\DatabaseTransactions;
use Illuminate\Database\Eloquent\ModelNotFoundException;

class RemoveLabelFromIssueTest extends TestCase
{
    use DatabaseTransactions;

    /** @test */
    public function it_removes_the_project_label_as_administrator(): void
    {
        ...
        $projectIssue->labels()->attach($projectLabel->id);
        $this->executeService($michael, $project, $projectLabel, $projectIssue);
    }

    /** @test */
    public function it_removes_the_project_label_as_hr(): void
    {
    ...
    /** @test */
    public function it_removes_the_project_label_as_normal_user(): void
    {
    ...
    public function it_fails_if_wrong_parameters_are_given(): void
    {
        ...
        (new RemoveLabelFromIssue)->execute($request);
    }
    ...
    private function executeService(Employee $michael, Project $project, ProjectLabel $label, ProjectIssue $issue): void
    {
        ...
        $this->assertDatabaseMissing('project_issue_project_label', [
            'project_label_id' => $label->id,
            'project_issue_id' => $issue->id,
        ]);
        ...
        Queue::assertPushed(LogAccountAudit::class, function ($job) use ($michael, $project, $label, $issue) {
            return $job->auditLog['action'] === 'project_label_removed_from_issue' &&
