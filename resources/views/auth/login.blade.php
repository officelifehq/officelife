    ...
    <div class="cf mt4 mw7 center br3 mb3 bg-white box">
      ...
      <div class="fn fl-ns w-50-ns pa3">
        <form method="POST" action="{{ route('login.store') }}">
          ...
          {{-- Email --}}
          <div class="">
            <label class="db fw4 lh-copy f6" for="email">{{ trans('auth.login_email') }}</label>
            <input type="email" name="email" class="br2 f5 w-100 ba b--black-40 pa2 outline-0" value="{{ old('email') }}" required>
          </div>

          {{-- Password --}}
          <div class="mb4">
            <label class="db fw4 lh-copy f6" for="password">{{ trans('auth.login_password') }}</label>
          ...
          <div class="">
            <div class="flex-ns justify-between">
              <div>
                <button class="btn add w-auto-ns w-100 mb2 pv2 ph3" name="save" type="submit" dusk="login-button">{{ trans('auth.login_cta') }}</button>
              </div>
            </div>
          </div>
      </form>
