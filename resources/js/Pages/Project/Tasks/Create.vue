    ...
    <div class="ph2 ph0-ns">
      ...
      <div class="mt4-l mt1 mw6 br3 bg-white box center breadcrumb relative z-0 f6 pb2">
        <ul class="list ph0 tc-l tl">
          ...
          <li class="di">
            <inertia-link :href="'/' + $page.props.auth.company.id + '/projects/' + project.id + '/tasks'">{{ project.name }}</inertia-link>
          </li>
          <li class="di">
            {{ $t('app.breadcrumb_project_create_task_list') }}
          </li>
        </ul>
      </div>
          ...
          <form @submit.prevent="submit">
            ...
            <div class="mv4">
              <div class="flex-ns justify-between">
                <div>
                  <inertia-link :href="'/' + $page.props.auth.company.id + '/projects/' + project.id + '/messages'" class="btn dib tc w-auto-ns w-100 mb2 pv2 ph3">
                    {{ $t('app.cancel') }}
                  </inertia-link>
                </div>
                <loading-button :classes="'btn add w-auto-ns w-100 mb2 pv2 ph3'" :state="loadingState" :text="$t('app.publish')" :cypress-selector="'submit-add-message-button'" />
              </div>
            </div>
          </form>
        </div>
