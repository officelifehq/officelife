      ...
      <div class="mw7 center br3 mb5 bg-white box restricted relative z-1">
        <div class="pa3 mt5 measure center">
          <h2 class="tc normal mb4">
            {{ $t('account.company_news_new_title', { name: $page.auth.company.name}) }}
          </h2>
          ...
          <form @submit.prevent="submit">
            ...
            <!-- Title -->
            <text-input :id="'title'" v-model="form.title" :name="'title'" :errors="$page.errors.title" :label="$t('account.company_news_new_title')" />

            <!-- Content -->
            <text-area v-model="form.content" :label="$t('account.company_news_new_title')" :datacy="'news-content'" />

            <!-- Actions -->
            <div class="mv4">
              <div class="flex-ns justify-between">
                <div>
                  <inertia-link :href="'/' + $page.auth.company.id + '/account/news'" class="btn btn-secondary dib tc w-auto-ns w-100 mb2 pv2 ph3">
                    {{ $t('app.cancel') }}
                  </inertia-link>
                </div>
