<template>
  <div class="di relative">
    <span v-if="employee.address" class="mt0 mb0" data-cy="employee-location">
      ...
      <inertia-link v-if="permissions.can_see_complete_address" :href="employee.url.edit" class="bb b--dotted bt-0 bl-0 br-0 pointer di f7 ml2">{{ $t('app.edit') }}</inertia-link>
    </span>

    <!-- case of no address set in profile -->
    <inertia-link v-if="!employee.address && permissions.can_edit_profile" :href="employee.url.edit" class="bb b--dotted bt-0 bl-0 br-0 pointer di f7">
