    ...
    <div class="ph2 ph5-ns">
      ...
      <div class="mw8 center br3 mb5 relative cf">
        ...
        <div class="fl w-30-l w-100 pl4-l">
          <!-- written by -->
          <h3 v-if="issue.author" class="ttc f7 gray mt0 mb2 fw4">
            {{ $t('project.message_show_written_by') }}
          </h3>

          <div v-if="issue.author" class="flex mb4">
            <div class="mr3">
              <avatar :avatar="issue.author.avatar" :size="64" :class="'br-100'" />
            </div>

            <div>
              <inertia-link :href="issue.author.url" class="mb2 dib">{{ issue.author.name }}</inertia-link>

              <span v-if="issue.author.role" class="db f7 mb2 relative">
                ...
                <ul class="list pa0 ma0">
                  ...
                  <li>
                    ...
                    <span class="gray">
                      {{ $t('project.members_index_role', { date: issue.author.added_at }) }}
                    </span>
                  </li>
                </ul>
              </span>
              <span v-if="issue.author.position && issue.author.role" class="db f7 gray">
                {{ $t('project.members_index_position_with_role', { role: issue.author.position.title }) }}
              </span>
              <span v-if="issue.author.position && !issue.author.role" class="db f7 gray">
                {{ issue.author.position.title }}
              </span>
            </div>
          </div>
          ...
          <p class="mt0 mb4">{{ issue.written_at }} <span class="f6 gray">({{ issue.written_at_human }})</span></p>
          ...
          <ul class="list pl0 ma0">
            <!-- edit -->
            <li class="mb2"><inertia-link :href="issue.url_edit" data-cy="project-edit" class="f6 gray">{{ $t('project.message_show_edit') }}</inertia-link></li>
...
export default {
  ...
  methods: {
    destroy() {
      axios.delete(`/${this.$page.props.auth.company.id}/company/projects/${this.project.id}/messages/${this.issue.id}`)
        .then(response => {
